...
public class LeetCode811 {
    public static void main(String[] args) {
        LeetCode811 leet = new LeetCode811();
        ...
        leet.subdomainVisits(s);
    }

    public List<String> subdomainVisits(String[] cpdomains) {
        Map<String, Integer> kvs = new HashMap<>();
        for (int i = 0; i < cpdomains.length; i++) {
            String[] cs = cpdomains[i].split(" ");
            int count = Integer.valueOf(cs[0]);
            String[] domains = cs[1].split("\\.");
            if (domains.length == 2) {
                kvs.put(domains[1], kvs.getOrDefault(domains[1], 0) + count);
                kvs.put(domains[0] + "." + domains[1], kvs.getOrDefault(domains[0] + "." + domains[1], 0) + count);
            }

            if (domains.length == 3) {
                kvs.put(domains[2], kvs.getOrDefault(domains[2], 0) + count);
                kvs.put(domains[1] + "." + domains[2], kvs.getOrDefault(domains[1] + "." + domains[2], 0) + count);
                kvs.put(domains[0] + "." + domains[1] + "." + domains[2], kvs.getOrDefault(domains[0] + "." + domains[1] + "." + domains[2], 0) + count);
            }
        }

        List<String> list = new ArrayList<>();
        for (String key : kvs.keySet()) {
            list.add(kvs.get(key) + " " + key);
        }
        return list;
    }
}
